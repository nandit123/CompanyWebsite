...
router.get('/dashboard',function(req,res){
    if(!req.session.companyUser) {
        ...
        var requestUrl = 'localhost:3333/product/getAllProducts?cId=' + req.session.companyUser;
        ...
        function callback(error, response, body) {
            ...
            var requestUrl2 = 'localhost:3333/review/getAllCompanyReviews?companyId=' + req.session.companyUser + "&type=" + 1;
            var options2 = {
                url: requestUrl2
            };
...
router.get('/reviews',function(req,res){
    if(!req.session.companyUser) {
        ...
        var requestUrl = 'localhost:3333/review/getAllCompanyReviews?companyId=' + req.session.companyUser;
        var options = {
            url: requestUrl
        };
        let rd = '';
...
router.get('/verifyAccount',function(req,res){
    ...
    var requestUrl = 'localhost:3333/companyUser/verifyAccount?email=' + email + '&code=' + code;
    var options = {
        url: requestUrl
    };
    let rd = '';
...
router.get('/settings',function(req,res){
    if(!req.session.companyUser) {
        ...
        var requestUrl = 'localhost:3333/companyUser/settings?cId=' + req.session.companyUser;
        var options = {
            url: requestUrl
        };
        let pd = '';
...
router.get('/currentProduct',function(req,res){
    if(!req.session.companyUser) {
        ...
        var requestUrl = 'localhost:3333/code/getCodes?cId=' + req.session.companyUser + '&productTitle=' + req.query.title;
        var options = {
            url: requestUrl
        };
        let codes = '';
